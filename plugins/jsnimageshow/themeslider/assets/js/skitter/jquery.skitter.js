...
(function($) {
	...
	$sk.fn.extend({
		...
		start: function()
		{
			...
			var cache_value = $('#'+self.settings.skitter_class_id+' .cache_skitter_index').attr('value');
			cache_value = parseInt(cache_value);
			if (!cache_value) cache_value = 0;
			$('.pp_count').text((cache_value+1) + this.settings.counter_separator_label + total);
		},
		...
		animationFade: function(options)
		{
			...
			for (i = 0;i<total;i++) {
				...
				box_clone.find('img').css({left:0,top:0});
				this.addBoxClone(box_clone);
