...
class JSNISGridDisplay extends JObject
{
	...
	function standardLayout($args)
	{
		...
		JHTML::stylesheet($this->_assetsPath.'css/' . 'prettyPhoto3.css',array('media'=>'screen','charset'=>'utf-8'));
		$this->loadjQuery();
		...
		JHTML::script($this->_assetsPath.'js/jquery/' . 'jquery.prettyPhoto2.js');
		...
		$html  = $pluginOpenTagDiv.'<div style="width: '.$width.'; height: '.$args->height.'px;border:none" class="jsn-themegrid-container '.$wrapClass.'"><div class="jsn-themegrid-container-long" style="height: '.$args->height.'px;"><div class="jsn-themegrid-container-site" style="height: '.$args->height.'px;">';
		$i=1;
		foreach ($images as $image)
		{
			...
			if ($i%6 == 1 && $i > 1) $html .= '</div><div class="jsn-themegrid-container-site" style="height: '.$args->height.'px;"><div class="clearfix"></div>';
			$html .= '<div id="'.$args->random_number.'_'.$i.'" class="jsn-themegrid-box jsn-themegrid-image">';
			$html .= '<a '.$href.' '.$openLinkIn.' '.$rel.' rev="'.htmlspecialchars(strip_tags(trim($image->description), '<b><i><s><strong><em><strike><u><br>')).'" title="'.htmlspecialchars($image->title).'" onfocus="blur()">';
			$html .= '<span class="jsn-themegrid-image-inner"><img id="img_'.$args->random_number.'_'.$i++.'" src="' . $image->$imageSource . '" border="0" alt="'.$image->title.'"/></span>';
			$html .= '<span class="thumb_title">'.$image->title.'</span><br /><span class="thumb_desc">'.trim($image->description).'</span>';
			$html .= '</a></div>';
		}
		$html .= '</div></div></div>'.$pluginCloseTagDiv;
