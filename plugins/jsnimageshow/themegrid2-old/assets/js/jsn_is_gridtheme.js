...
(function($) {
	$.fn.extend( {
		...
		initLayout:function(container,layout,thumbnail_width,thumbnail_height){
			...
			switch(layout){
				case 'fixed':
					...
					container.find(".jsn-themegrid-box").css({width:width_box,height : height_box});
					container.find(".jsn-themegrid-image-inner").css({width:width_inner,height : height_inner});
		...
		initThumbnailSpace:function(container,thumbnail_space){
			...
			var margin_left = parseInt(thumbnail_space)+12;
			for(var i=1;i<=numberImg;i++){
				key = (themeId=='')?i:themeId+'_'+i;
				document.getElementById(key).style.marginLeft = margin_left+"px";
