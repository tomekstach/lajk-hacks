...
class plgjsnimageshowThemeGrid2InstallerScript
{
	...
	private function _updateSchema()
	{
		...
		$eid = $row->find(array('element' => 'themegrid2', 'type' => 'plugin'));
		if ($eid)
		{
			$db = JFactory::getDBO();
	...
	private function _getInfo($id)
	{
		...
		$query->where('element=\'themegrid2\' AND type=\'plugin\' AND folder=\'jsnimageshow\' AND extension_id = ' . $id);
		$db->setQuery($query);
