...
//    $stopdropdown = ($item->level > 120) ? '-nodrop' : '';
	...
	if ($item->ftitle != "") {
		$astitle = explode('-', $item->ftitle);
		$astitleurl = $item->ftitle = $astitle[0];
		if (!empty($astitle[1])) $astitleurl .= '<br /><span>'.$astitle[1].'</span>'; 
		...
		if ($item->menu_image) {
			...
			if ($item->params->get('menu_text', 1 ) AND !$params->get('imageonly', '0')) {
				switch ($params->get('menu_images_align', 'default')) :
					default:
										case 'default':
											$linktype = '<img src="'.$item->menu_image.'" alt="'.$item->ftitle.'" align="left"'.$imagerollover.'/><span class="titreck">'.$astitleurl.'</span> ' ;
										break;
										case 'bottom':
						$linktype = '<span class="titreck">'.$astitleurl.'</span><img src="'.$item->menu_image.'" alt="'.$item->ftitle.'" style="display: block; margin: 0 auto;"'.$imagerollover.' /> ' ;
					break;
					case 'top':
		...
		switch ($item->type) :
			default:
				...
				break;
			case 'separator':
				echo $opentag.'<span class="separator ' . $item->anchor_css . '">' . $astitleurl . '</span>'.$closetag;
				break;
			case 'heading':
