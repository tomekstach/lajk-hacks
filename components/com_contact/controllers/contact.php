...
class ContactControllerContact extends JControllerForm
{
	...
	private function _sendEmail($data, $contact, $copy_email_activated)
	{
		...
		// AstoSoft - Start
		$zgoda1  = intval($data['contact_zgoda_przetwarzanie']) == 0 ? 'NIE' : 'TAK';
		$zgoda2  = intval($data['contact_zgoda_przesylanie']) == 0 ? 'NIE' : 'TAK';

		// Prepare email body
		$prefix = JText::sprintf('COM_CONTACT_ENQUIRY_TEXT', JUri::base());
		$body   = $prefix . "\n" . $name . ' <' . $email . '>' . "\r\n\r\n";
		$body		.= 'Zgoda przetwarzanie: '.$zgoda1."\r\n";
		$body		.= 'Zgoda przesyłanie: '.$zgoda2."\r\n\r\n";
		...
		$sent = $mail->Send();
			...
			unset($mail);
			$mail = JFactory::getMailer();
			$mail->addRecipient($contact->country);
			$mail->addReplyTo($email, $name);
			$mail->setSender(array($mailfrom, $fromname));
			$mail->setSubject($sitename . ': ' . $subject);
			$mail->setBody($body);
