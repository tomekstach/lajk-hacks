...
class JSNFormGenerateHelper
{
		...
		foreach ($getContainer as $items)
		{
			if ($items)
			{
				...
				foreach ($items as $item)
				{
					...
					// AstoSoft
                    $columnOutput .= "<div class=\"jsn-container-{$columName}\">";
					if (!empty($dataColumn))
					{
	...
	public static function fieldFileUpload($data, $dataSumbission)
	{
		...
		$required = !empty($data->options->required) ? '<span class="required">*</span>' : '';
		// AstoSoft
		$required_label = !empty($data->options->required)?' style="font-size:35px;font-family: \'Optima\';"':' style="font-family: \'Optima\';"';
		$instruction = !empty($data->options->instruction) ? " <i original-title=\"" . htmlentities(JText::_($data->options->instruction), ENT_QUOTES, "UTF-8") . "\" class=\"icon-question-sign\"></i>" : '';
		// AstoSoft
        $html = "<div class=\"control-group {$customClass} {$identify} {$hideField}\" data-id=\"{$data->id}\"><label  class=\"control-label\"{$required_label}>" . htmlentities(JText::_($data->label), ENT_QUOTES, "UTF-8") . "</label><div class=\"controls {$requiredBlank}\"><input id=\"{$data->id}\" class=\"input-file\" name=\"{$data->id}[]\" {$multiple} type=\"file\" /></div><div class=\"clear_20\"></div></div>";
		return $html;
	}
